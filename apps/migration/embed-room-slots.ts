 ...
 * generated on the sculpture surface — the algorithm is a 1:1 port of
 * `apps/museum/src/components/museum/three/auto-slots.ts` (same seed = room
 * id), so embedded slots land exactly where the builder's runtime fallback
 * previews them today. Keep the two in sync if you change either.
 *
 * Usage:
...
// ============================================================================
// Slot generation — 1:1 port of apps/museum/.../auto-slots.ts (keep in sync!)
// ============================================================================
...
/** Triangle soup in scene-root space, from the glTF document (world transforms applied). */
function collectSurface(doc: Document): Surface | null {
...
function generateSlots(surface: Surface, count: number, seed: number): GeneratedSlot[] {
  if (count <= 0) return [];
...
function countAuthoredSlots(doc: Document): number {
  let n = 0;
