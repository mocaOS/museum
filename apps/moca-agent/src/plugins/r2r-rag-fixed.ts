...
export class R2RService extends Service {
  ...
  async generateSearchSummary(searchResults: R2RSearchResult[], query: string, runtime: IAgentRuntime): Promise<string> {
    try {
      ...
      const aiSummary = await runtime.useModel(ModelType.TEXT_SMALL, {
        prompt,
        temperature: 0.2,
        maxTokens: 600,
      });

      const sourceList = this.formatDetailedSources(searchResults);
